...
function ForceGraph({ w, h, charge, problemData, gadgetMap }) {
  ...
  function highlightGadget(nodeId, gadgetMap) {
    ...
    gadgetMap.forEach((item) => {
      if (
        (item.reductionFromIds.includes(nodeId) || item.reductionToIds.includes(nodeId)) &&
  ...
  useEffect(() => {
    ...
    problemData.links?.forEach((d, i) => {
      if (d.directed) {
        ...
        marker.append("path").attr("d", "M0,-5L10,0L0,5").attr("fill", getColorByKey("Edges"));
    ...
    const link = svg
      ...
      .style("stroke", (d) =>
        getDelayMs(d) > 0 ? getColorByKey("Edges") : getColorByKey(d.color || "Edges"),
      )
      .style("stroke-width", "2px")
      .style("stroke-dasharray", (d) => (d.dashed ? "5,5" : "none"));

    link
      .filter((d) => getDelayMs(d) > 0 && d.color)
      .transition()
    ...
    const node = svg
      ...
      .attr("fill", (d) =>
        getDelayMs(d) > 0 ? getColorByKey("Background") : getColorByKey(d.color || "Background"),
      )
    ...
    // Force simulation
    const simulation = d3
      .forceSimulation(problemData.nodes)
      .force(
        "link",
        d3
          .forceLink(problemData.links)
          .id((d) => d.name)
...
export default function StandardGraphSvgReact(props) {
  ...
  return <ForceGraph w={700} h={700} charge={charge} {...props} />;
}
